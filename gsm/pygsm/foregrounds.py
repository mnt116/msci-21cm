...
n = np.zeros(npix) # initial sky arrayy
...
sky_times = [[0,12],[12,23]] # Define intervals to take measurements over
plt.figure()
for i in range(len(sky_times)):
    freqs = np.linspace(50,99,50) # EDGES measures from 50 to 99 MHz and takes 50 measurements
    temps = []
    for j in freqs:
        value = int_sky(freq=j, filter_array=filter_array, location=('-26.69719','116.63903',373), sampling_rate=30, interval=sky_times[i], nside=nside, plot_figs=False)
        temps.append(value)
    plt.plot(freqs,temps,'b-',label='%i_foreground' % sky_times[i][0])
    data = np.array([freqs, temps])
    np.savetxt("%i_foreground_sim.txt" % sky_times[i][0], data, delimiter = ",") # save data in the same style as EDGES
plt.legend()
plt.savefig('foreground_sim.png')
